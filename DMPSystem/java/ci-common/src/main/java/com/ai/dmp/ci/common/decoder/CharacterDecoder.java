...
package com.ai.dmp.ci.common.decoder;
...
public abstract class CharacterDecoder 
{
	...
	//自己添加可以指定charset的decode方法，为了避免在不同默认charset编码环境下分别进行加密和解密的乱码问题。
	public byte decodeBuffer(String inputString,String charset)[] throws IOException
	{
		byte inputBuffer[] = new byte[inputString.length()];
		ByteArrayInputStream inStream;
		ByteArrayOutputStream outStream;
		inputBuffer = inputString.getBytes(charset);
//		inputString.getBytes(0, inputString.length(), inputBuffer, 0);
		inStream = new ByteArrayInputStream(inputBuffer);
		outStream = new ByteArrayOutputStream();
		decodeBuffer(inStream, outStream);
